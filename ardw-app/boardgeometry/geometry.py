...
def get_chamfered_rect(size, radius, chamfpos=0, chamfratio=0):
    ...
    if chamfpos & 4:
        points = np.append(
            points,
            [(x, y + height - offset), (x + offset, y + height)],
            axis=0
        )
    else:
        points = np.append(
            points,
            get_arc(x + radius, y + height - radius, radius, 180, 90),
            axis=0
        )
    ...
    if chamfpos & 8:
        points = np.append(
            points,
            [(x + width - offset, y + height), (x + width, y + height - offset)],
            axis=0
        )
    else:
        points = np.append(
            points,
            get_arc(x + width - radius, y + height - radius, radius, 90, 0),
            axis=0
        )
    ...
    if chamfpos & 2:
        points = np.append(
            points,
            [(x + width, y + offset), (x + width - offset, y)],
            axis=0
        )
    else:
        points = np.append(
            points,
            get_arc(x + width - radius, y + radius, radius, 0, -90),
            axis=0
        )
    ...
    if chamfpos & 1:
        ...
        points = np.append(
            points,
            get_arc(x + radius, y + radius, radius, 270, 180),
            axis=0
        )
