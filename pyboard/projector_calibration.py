...
@prt.transformer
def pointGuidance(probePosition):

    global checkerBoard
    global imagePoints
    global capturedWorldPoints
    ...
    for row in range(0, gridSize[0]+1):
        for col in range(0, gridSize[1]+1):
            ...
            capturedWorldPoints[0:3, row*4+col] = probePosition
...
@prt.transformer
def projectPointOnProbeTip(probePosition):
    k = -1
    global projectPointOnProbeTip
    global projectionMatrix
    global currentWorldPoint


    while (k == -1):
        #TODO
        currentWorldPoint = np.array(probePosition)

        currentPixelPoint = np.matmul(projectionMatrix.T, currentWorldPoint.T)
...
def projector(tip_pos):
    makeCheckerboard()
    pointGuidance(tip_pos)

    projectPointOnProbeTip(tip_pos)
